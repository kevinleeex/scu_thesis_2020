% 文件名：Manual.tex
% 文件描述：以 scuthesis 四川大学学位论文文档类为基础的 LaTeX 模版说明文档
% 作者：LegendaryLeo
% 修改日期：2016年6月3日
\documentclass[master,UTF8,hyperref]{../Template/scuthesis}
\begin{document}
\title{四川大学学位论文~\LaTeX~模版 Ver. 0.1}
\ENGtitle{The SCU Dissertation \LaTeX ~ Class Ver. 0.1}
\author{Legendary L.}
\ENGauthor{Legendary L.}
\accomplishdate{\the\year~年~\the\month~月~\the\day~日}
\school{电子信息学院}
\supervisor{某某某\quad教授}
\ENGsupervisor{Prof. Anonymous}
\major{XXXX}
\ENGmajor{Something and Something}
\direction{XXXX}
\ENGdirection{XXXX}
\defensedate{XXXX~年~XX~月~XX~日}

\keywords{论文，文档排版，\LaTeX，\CTeX}
\ENGkeywords{Dissertation, Document Typesetting, \LaTeX, \CTeX}

\maketitle
\frontmatter\pagenumbering{Roman}\pagestyle{fancy}\makechaptertitlecenter
\include{chaps/CopyrightStatement}
\include{chaps/0_0_Abstract}
\include{chaps/0_1_Abbreviations}
\include{chaps/0_2_Symbols}

\maketoc

\mainmatter\pagenumbering{arabic}\pagestyle{fancy}\makechaptertitleleft

\include{chaps/1_Introduction}
\include{chaps/2_Chapter2}


\backmatter\makechaptertitlecenter
\begin{reference}
	\bibliographystyle{../Template/chinesebst}\bibliography{Manualbib}
\end{reference}

\include{chaps/Achievements}
\include{chaps/OriginalStatement}
\include{chaps/CopyrightAuthorization}
\include{chaps/Thanks}
\end{document}
